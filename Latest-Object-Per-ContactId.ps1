...
# Business rule: exclude RecordTypes with these ID specified by Gracia. 
...
Import-Csv $raw_csv | Where-Object RecordTypeId -NotIn $ExcludeRecordTypeIdList |
    Select-Object ContactId,Id,CampaignId,AccountId,Payment_Id__c,MDNSW_PaymentID__c,CreatedDate,Type,Name,Amount,RecordTypeId |
    Export-Csv -Path $clean_csv -NoTypeInformation # save this between PS sessions

$indata = Import-Csv -Path $clean_csv # reload from disk

# $indata | Group-Object -Property RecordTypeId


# NOTE: This calculation uses the same filtered subset after excluding 
